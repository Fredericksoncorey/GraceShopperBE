...
describe('Database', () => {
  ...
  describe('Users', () => {
    ...
    let userCredentials = {username: 'billybob', password: 'bobbybadboy', email: 'billibobjoe@at&t.com', isAdmin: false};
  ...
  describe('getUserByUsername', () => {
    it('Gets a user based on the user Id', async () => {
      ...
      expect(user).toBeTruthy();
      expect(user.id).toBe(userToCreateAndUpdate.username);
    })
  })
})
...
describe ('Products', () => {
    let queriedProduct
    let productCredentials = {title: 'Thunder', artist:'Cloudstrikk', genre: 'metal', releaseDate:'2012-11-22', description:'12 songs from there hit album "Thunder"', price: 12, quantity: 31}
    describe('createProduct', () => {
        beforeAll(async () => {
            productToBeCreatedAndUpdate = await createProduct(productCredentials);
            const {rows} = await client.query(`SELECT * FROM products WHERE title = $1`, [productCredentials.title]);
            queriedProduct = rows[0];
          })
          it('Creates the product', async () => {
            expect(productToBeCreatedAndUpdate.title).toBe(productCredentials.title);
            expect(queriedProduct.title).toBe(productCredentials.title);
          });
    ...
    describe('Update Product Quantity', () =>{
        describe('updateProduct', () => {
            let queriedProduct;
            beforeAll(async() => {
              productToCreateAndUpdate = await updateProduct({title: 'Thunder', artist:'Cloudstrikk', genre: 'metal', releaseDate:'2012-11-22', description:'12 songs from there hit album "Thunder"', price: 12, quantity: 31});
              queriedProduct = await getProductById(productToCreateAndUpdate.id);
            })
